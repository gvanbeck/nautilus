...
func TestBarChart_Draw(t *testing.T) {
	doc := newDoc(t)
	opts := chart.Options{FontName: "regular", FontSize: 9,
		Title: &chart.Title{Text: "Products"},
		XAxis: &chart.Axis{}, Legend: &chart.Legend{Enabled: chart.Bool(false)},
		YAxis: &chart.Axis{Categories: []string{"A","B","C","D"}},
		Series: []chart.Series{{Name: "Units", Data: []float64{143,112,98,87}}},
	}
	if err := (&bar.BarChart{Options: opts}).Draw(doc, 10, 10, 400, 250); err != nil { t.Fatalf("Draw: %v", err) }
	assertValidPDF(t, doc)
}
func TestBarChart_EmptySeries(t *testing.T) {
	doc := newDoc(t)
	if err := (&bar.BarChart{Options: chart.Options{FontName: "regular"}}).Draw(doc, 10, 10, 400, 250); err != nil { t.Fatalf("Draw empty: %v", err) }
}
